...
#include "MathsCustomFunctions.hpp"
    ...
    double CellICCSMCFromCellML::GetIntracellularCalciumConcentration()
    {
        return mStateVariables[1];
    }
    CellICCSMCFromCellML::CellICCSMCFromCellML(boost::shared_ptr<AbstractIvpOdeSolver> pSolver, boost::shared_ptr<AbstractStimulusFunction> pIntracellularStimulus)
        : AbstractCardiacCell(
                pSolver,
                20,
                0,
                pIntracellularStimulus)
    {
        ...
        //
        this->mpSystemInfo = OdeSystemInformation<CellICCSMCFromCellML>::Instance();
        Init();
    ...
    CellICCSMCFromCellML::~CellICCSMCFromCellML()
    {
    }

    
    double CellICCSMCFromCellML::GetIIonic(const std::vector<double>* pStateVariables)
    {
        ...
        const double var_ICC_neural__D = 1.3; // conductance_units
        const double var_ICC_neural__E = 7.6264172646662596; // conductance_units
        const double var_ICC_neural__F = 0.140779519087363; // dimensionless
        const double var_ICC_neural__gbk_neur = var_ICC_neural__E * tanh(var_ICC_neural__F * mParameters[5]) + var_ICC_neural__D; // conductance_units
    ...
    void CellICCSMCFromCellML::EvaluateYDerivatives(double var_chaste_interface__Time__time, const std::vector<double>& rY, std::vector<double>& rDY)
    {
        // Inputs:
        // Time units: millisecond
        double var_chaste_interface__Membrane__V_m = (mSetVoltageDerivativeToZero ? this->mFixedVoltage : rY[0]);
        ...
        const double var_ICC_neural__A = 6.2000000000000003e-5; // millimolar_per_time_units
        const double var_ICC_neural__B = 0.0025394316137978298; // millimolar_per_time_units
        const double var_ICC_neural__C = 0.27644063163584098; // dimensionless
        const double var_ICC_neural__beta_neur = var_ICC_neural__B * tanh(var_ICC_neural__C * mParameters[4]) + var_ICC_neural__A; // millimolar_per_time_units
        ...
        const double var_SMC_neural__G = 1.0000000000000001e-5; // millimolar
        const double var_SMC_neural__H = 0.0017797296102471001; // millimolar
        const double var_SMC_neural__I = 0.396177113350488; // dimensionless
        const double var_SMC_neural__Ach = var_SMC_neural__H * tanh(var_ICC_neural__stim_ach * var_SMC_neural__I) + var_SMC_neural__G; // millimolar
        ...
        const double d_dt_chaste_interface_var_SM_Membrane__Vm_SM = -(-var_I_stim__I_stim + var_I_BK_SM__I_BK_SM + var_I_LVA_SM__I_LVA_SM + var_I_Ltype_SM__I_Ltype_SM + var_I_NSCC_SM__I_NSCC_SM + var_I_Na_SM__I_Na_SM + var_I_bk_SM__I_bk_SM + var_I_ka_SM__I_ka_SM + var_I_kr_SM__I_kr_SM) / var_SM_Membrane__Cm_SM; // voltage_units / time_units
    ...
    std::vector<double> CellICCSMCFromCellML::ComputeDerivedQuantities(double var_chaste_interface__Time__time, const std::vector<double> & rY)
    {
        // Inputs:
        // Time units: millisecond
        double var_chaste_interface__SM_Membrane__Ca_i = rY[1];
...
template<>
void OdeSystemInformation<CellICCSMCFromCellML>::Initialise(void)
{
...
CHASTE_CLASS_EXPORT(CellICCSMCFromCellML)
